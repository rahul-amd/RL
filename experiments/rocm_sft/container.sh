...
exec singularity exec --rocm --cleanenv --containall --pwd "$REPO" \
  ...
  "${NRL_ROCM_IMAGE:-/shared_silo/scratch/containers/primus_v26.5-pytorch2.12-te2.15.sif}" \
  env PYTHONPATH="${NRL_ROCM_SFT_PACKAGES:-$REPO/experiments/rocm_sft/packages}:$REPO" \
