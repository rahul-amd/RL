...
def job_states(jobs):
    ...
    for row in output.splitlines():
        ...
        if int(job) not in jobs:
            continue
        if user != getpass.getuser() or not name.startswith("tml-opd-"):
...
def submit(state, config, gpus):
    assert gpus == 32, "This recovery monitor expects the validated 32-GPU topology"
    checkpoint, step = latest_checkpoint(
        EXPERIMENT / f"run_opd_{config['run_id']}" / "checkpoints"
    )
    script = "opd_multinode.sbatch" if gpus == 32 else "opd.sbatch"
    export = f"ALL,TML_OPD_RUN_ID={config['run_id']},TML_OPD_CONFIG={checkpoint / 'config.yaml'}"
...
def submit_evaluations(state, config, checkpoint_step):
    for step in config["evaluation_steps"]:
        key = str(step)
        if checkpoint_step < step or key in state["evaluations"]:
            continue
        adapter = (
            EXPERIMENT
            / f"run_opd_{config['run_id']}/checkpoints/step_{step}/policy/weights/model"
        )
        if not (adapter / "adapter_model.safetensors").is_file():
            raise FileNotFoundError(adapter)
        state["evaluations"][key] = {"status": "submitting"}
        persist(config["state_path"], state)
        try:
            output = command(
                [
                    "sbatch",
                    "--array=0",
                    f"--job-name=tml-opd-eval-{step}",
                    f"--export=ALL,TML_EVAL_ADAPTER={adapter}",
                    str(EXPERIMENT / "eval_aime.sbatch"),
                ]
            )
        except (subprocess.CalledProcessError, subprocess.TimeoutExpired):
            state["status"] = "needs_attention"
            event(
                state,
                "Evaluation submission failed or uncertain; inspect Slurm before retrying.",
            )
            persist(config["state_path"], state)
            raise
        match = re.search(r"Submitted batch job (\d+)", output)
        if not match:
            raise RuntimeError(
                f"Uncertain evaluation submission; inspect Slurm: {output}"
            )
        job = int(match[1])
        state["evaluations"][key] = {
            "status": "submitted",
            "job": job,
            "output": str(EXPERIMENT / f"run_aime_{job}"),
        }
        event(state, f"Submitted checkpoint-{step} AIME24 evaluation as {job}")
        persist(config["state_path"], state)


def observe_evaluations(state):
    for step, evaluation in state["evaluations"].items():
        if evaluation["status"] in {"complete", "needs_attention"}:
            continue
        output = command(
            [
                "sacct",
        ...
        statuses = {
            row.strip().rstrip("|").split()[0]
            for row in output.splitlines()
            if row.strip()
        }
        evaluation["slurm_states"] = sorted(statuses)
        if statuses == {"COMPLETED"}:
            summary_path = Path(evaluation["output"]) / "opd-summary.json"
            summary = json.loads(summary_path.read_text())
            assert summary["samples"] == 960
            evaluation.update(status="complete", results=summary)
            event(
                state,
                f"Checkpoint-{step} AIME24 complete: accuracy={summary['accuracy']:.4f}",
            )
        elif statuses and not statuses <= ACTIVE:
            evaluation["status"] = "needs_attention"
            event(
                state,
                f"Checkpoint-{step} AIME24 ended {sorted(statuses)}; inspect its logs",
            )


def tick(state, config):
    now = time.time()
    run = EXPERIMENT / f"run_opd_{config['run_id']}"
    _, checkpoint_step = latest_checkpoint(run / "checkpoints")
    submit_evaluations(state, config, checkpoint_step)
    ...
    if upgrade and upgrade_status == "RUNNING" and status != "UNKNOWN":
        upgrade_log = EXPERIMENT / f"opd-{upgrade}.log"
    ...
    log_path = EXPERIMENT / f"opd-{active}.log"
    ...
    if status == "RUNNING":
        ...
        if status == "COMPLETED" and checkpoint_step >= config["target_steps"]:
            ...
            evaluations = [
                state["evaluations"][str(step)] for step in config["evaluation_steps"]
            ]
            if all(
                ...
                state["status"] = (
                    "complete"
                    if all(item["status"] == "complete" for item in evaluations)
                    else "needs_attention"
                )
                event(
                    state, f"OPD and evaluation monitoring finished: {state['status']}"
                )
                return False
            event(state, "OPD training complete; waiting for AIME24 evaluations")
            return True
        recoverable = (
