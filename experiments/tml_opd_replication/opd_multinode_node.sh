...
cd "$REPO"
if [ "${1:-}" != --inside ]; then
  test "$SLURM_GPUS_ON_NODE" -eq 8
  TASK_TMP=/tmp/tml_opd_${SLURM_JOB_ID}
  mkdir -p "$TASK_TMP"
  ...
  exec bash experiments/tml_opd_replication/container.sh env \
    ...
    TML_OPD_RUN_ID="$TML_OPD_RUN_ID" TML_OPD_CONFIG="$TML_OPD_CONFIG" \
    bash experiments/tml_opd_replication/opd_multinode_node.sh --inside "$@"
fi
shift
...
if [ "$TML_NODE_RANK" -eq 0 ]; then
  ...
  "${UV[@]}" python experiments/tml_opd_replication/run_opd.py \
    --config "$TML_OPD_CONFIG" \
    "cluster.num_nodes=$TML_NODE_COUNT" "cluster.gpus_per_node=$TML_GPU_COUNT" \
    "policy.dtensor_cfg.dp_replicate_size=$((TML_NODE_COUNT / 2))" \
    "teacher.dtensor_cfg.dp_replicate_size=$((TML_NODE_COUNT / 2))" \
    "policy.generation.colocated.resources.num_nodes=$((TML_NODE_COUNT / 2))" \
    "policy.generation.colocated.resources.gpus_per_node=$TML_GPU_COUNT" \
    "logger.log_dir=experiments/tml_opd_replication/run_opd_${TML_OPD_RUN_ID}/logs" \
    "logger.wandb.name=tml-opd-r128-${TML_OPD_RUN_ID}" \
    "logger.wandb.id=tml-opd-r128-${TML_OPD_RUN_ID}" \
    "checkpointing.checkpoint_dir=experiments/tml_opd_replication/run_opd_${TML_OPD_RUN_ID}/checkpoints" "$@"
  touch "experiments/tml_opd_replication/run_opd_${TML_OPD_RUN_ID}/completed_${TML_RAY_PORT}"
else
  "${UV[@]}" python - <<'WAIT'
import os
import socket
...
WAIT
  ...
  COMPLETED="experiments/tml_opd_replication/run_opd_${TML_OPD_RUN_ID}/completed_${TML_RAY_PORT}"
